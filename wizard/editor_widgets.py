...
class SignalConnection(gui.HBox):
    def __init__(self, widget, listenersList, eventConnectionFuncName, eventConnectionFunc, **kwargs):
        ...
        self.dropdownListeners = gui.DropDown(width='22%', height='100%')
        ...
        #
        # self.eventMethods = gui.Input(width='64%', height='100%')
        # self.eventMethods.onmousedown.do(self.on_mousedown)
        self.dropdownMethods = gui.DropDown(width='42%', height='100%')
        ...
        for w in listenersList:
            if w.variable_name == 'App':  # 限制App，旨为在Editor类中添加事件函数
        ...
        if self.eventConnectionFunc.callback is not None:
            try:
                ...
                print('----------SignalConnection------------')
                print(eventConnectionFuncName)
        ...
        # self.append([self.label, self.eventMethods])
    ...
    def on_connection(self, widget, dropDownValue):
        ...
        if hasattr(self.refWidget, "eventHandler") is False:
            print("this eventHandlerList is not existed. will setattr")
            setattr(self.refWidget, "eventHandler", {})
        self.refWidget.eventHandler[self.eventConnectionFuncName] = self.dropdownMethods.get_value()
        print(self.refWidget.eventHandler)
...
class SignalConnectionManager(gui.Container):
    ...
    def build_widget_list_from_tree(self, node):
        ...
        for child in node.children.values():
            if hasattr(child, 'attributes') and (hasattr(child, 'variable_name') and child.variable_name is not None):
    ...
    def update(self, widget, widget_tree):
        ...
        """
        print(">>>>>>>>>>>>>>>>START: SignalConnectionManager.update()>>>>>>>>>>>>>>>>")
        self.label.set_text('Signal connections: ' + widget.variable_name)
        if widget.variable_name == 'stage':
            self.label.set_text('No signal connections for the selected widget: ' + widget.variable_name)
            return
        self.listeners_list = []
        ...
        for (setOnEventListenerFuncname, setOnEventListenerFunc) in inspect.getmembers(widget):
            ...
            if issubclass(type(setOnEventListenerFunc), gui.ClassEventConnector):
                self.container.append(SignalConnection(widget,
                                                       ...
                                                       width='100%'))
        print("<<<<<<<<<<<<<<<<End:   SignalConnectionManager.update()<<<<<<<<<<<<<<<<")
...
class WidgetCollection(gui.Container):
    def __init__(self, appInstance, **kwargs):
        ...
        self.widgetsContainer = gui.HBox(width='100%', height='96%')
