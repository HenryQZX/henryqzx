...
class PoseDetection:
    ...
    def __init__(self):
        model_path = '/home/wizard/cv/models/pose_detection.tflite'
        model_pose = '/home/wizard/cv/models/pose_landmark_upper_body.tflite'
        ...
        log('gpu:', tflite.NNModel(model_pose, inShape, outShape, 4, 0))

        self.POSE_CONNECTIONS = [(0, 1), (1, 2), (2, 3), (3, 7),
                                 (0, 4), (4, 5), (5, 6), (6, 8),
                                 (9, 10),
    ...
    def process(self, image_roi):
        ...
        detections = self._tensors_to_detections(bboxes, scores, self.anchors)
        normalized_pose_detections = self.py_cpu_nms(detections, 0.3)

        normalized_pose_detections = np.stack(normalized_pose_detections) if len(
            normalized_pose_detections) > 0 else np.zeros((0, 12 + 1))
        pose_detections = self.denormalize_detections(normalized_pose_detections, scale, pad)
        if len(pose_detections) > 0:
            xc, yc, scale, theta = self.detection2roi(pose_detections)
            img, affine, box = self.extract_roi(frame, xc, yc, theta, scale)
            ...
            landmarks = self.denormalize_landmarks(normalized_landmarks, affine)

            self.draw_roi(image_roi, box)
            for i in range(len(flags)):
                landmark, flag = landmarks[i], flags[i]
                if flag > .5:
                    self.draw_landmarks(image_roi, landmark[:, :2], self.POSE_CONNECTIONS, size=2)
        return image_roi
    ...
    def extract_roi(self, frame, xc, yc, theta, scale):
        ...
        for i in range(points.shape[0]):
            pts = points[i, :, :3].T
            log('pts', pts.shape, points1.shape, pts.dtype, points1.dtype)
    ...
    def draw_landmarks(self, img, points, connections=[], color=(255, 255, 0), size=2):
        for point in points:
            x, y = point
            x, y = int(x), int(y)
